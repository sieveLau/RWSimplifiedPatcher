...
int main(int argc, char **argv) {
...
#endif
    std::map<std::string,simplexml::translation_unit *> output_map;

    auto language_dir_prefix = getDirectoryPrefix();
    std::string input;
    std::string output_dir_from_user;
    switch (argc) {
    case 1:
    case 2:
        std::cout << "outputDir: ";
        std::getline(std::cin, output_dir_from_user);
        std::cout << "DefsDir: ";
        std::getline(std::cin, input);
        break;
    case 3:
        input = std::string(argv[2]);
        output_dir_from_user = std::string(argv[1]);
        break;
    ...
    std::ofstream output;
    output.open(output_definjected_destination / "pairs.txt", std::ios_base::trunc);
    if (output.is_open()) {
        for (auto &&key : keys) {
            output << key << ":\n";
        }
